...
;(function ($, window, document, undefined) {
    ...
    page.prototype.ajaxdata = function (options) {
        ...
        query_args += 'page=' + options.nowpage;
        if (options.pagesize > 0) {
            query_args += '&page_size=' + options.pagesize;
        }
        if (options.total > 0) {
        ...
        if (typeof call_service === "function") {
            options
            var _this = this;
            call_service(
                url, true, 'GET', ''
    ...
    page.prototype.clickHandler = function (event) {
        ...
        if ($(".sicon-checkbox").length > 0) {
            $(".sicon-checkbox").removeClass("active");
        }
